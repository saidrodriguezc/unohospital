<?PHP
  ...
  if(($opcion == "guardareditado")||($opcion == "guardarnuevo"))
  {
	...
	{
        $vsql = "INSERT INTO lineasprod(codigo,descripcion,creador,momento) values('".$codigo."', 
	            '".$descripcion."','".$_SESSION['USERNAME']."',CURRENT_TIMESTAMP)";
    
		$cant = $clase->EjecutarSQL($vsql);
	
		if($cant == 1)
    		$clase->Aviso(1,"Linea de Productos creada Exitosamente");  	
		else
	...
	{
        $vsql = "UPDATE lineasprod SET codigo = '".$codigo."' , descripcion = '".$descripcion."' ,
	             ...
	             WHERE lineaprodid=".$lineaprodid;
    
	    $clase->EjecutarSQL($vsql);
	
   		$clase->Aviso(1,"Linea de Productos modificada Exitosamente");  			  
    }	
  ...
  if($opcion == "nuevo")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/lineasprod.png" width="32" height="32" border="0"> </td>
				 <td width="520"> Nueva Linea de Productos <td>
				 <td>  <a href="lineas.php"> Listado de Lineas de Productos </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
	...
	$cont.='<br><br><br><center>
	        ...
	        <input type="hidden" name="lineasprodid" value="'.$id.'">
			<table width="400">
	         <tr> 
  ...
  if($opcion == "detalles")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/lineasprod.png" width="32" height="32" border="0"> </td>
				 <td width="510"> Lineas de Productos <td>
				 <td>  <a href="lineas.php"> Listado de Lineas de Productos </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
  	...
  	$vsql = "SELECT * FROM lineasprod WHERE lineaprodid=".$id;
	...
	if($row = mysql_fetch_array($result))
	{
		$cont.='<br><br><br><center>
	        ...
	        <input type="hidden" name="lineaprodid" value="'.$id.'">
			<table width="400">
	         <tr> 
			  <td> <label class="Texto15"> Codigo : </label> </td>
  ...
  {
    $id = $_GET['id'];
    $vsql = "DELETE FROM lineasprod WHERE lineaprodid=".$id;
	$clase->EjecutarSQL($vsql);
	$clase->Aviso(3,"Linea de Productos eliminada Exitosamente");  		
	header("Location: lineas.php");
  }
  ...
  if($opcion == "encontrar")
  {
    ...
    $vsql = "SELECT * FROM lineasprod WHERE codigo like '%".$criterio."%' OR descripcion like '%".$criterio."%' ORDER BY descripcion ASC Limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    $_SESSION['SQL_LINEAS'] = $vsql;
	header("Location: lineas.php");
  }
  ...
  {
    $criterio = $_POST['criterio'];
  	$vsql = "SELECT * FROM lineasprod ORDER BY descripcion ASC limit 0,30";
	$_SESSION['SQL_LINEAS'] = "";
	header("Location: lineas.php");
  }
  ...
  if($opcion == "")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/lineasprod.png" width="32" height="32" border="0"> </td>
				 <td width="400"> Lineas de Productos <td>
    ...
    $vsql = $_SESSION['SQL_LINEAS'];
	if($vsql == "")
    	$vsql = "SELECT * FROM lineasprod ORDER BY descripcion ASC limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    ...
    while($row = mysql_fetch_array($result)) 
	{
		 ...
		 $cont.=' <td> </td>
				  ...
				  <td width="100"> </td>
				  <td> <a href="?opcion=detalles&amp;id='.$row['lineaprodid'].'"> <img src="images/seleccion.png" border="0"> </td>				  
				 </tr>';
	}
	$cont.='</table>
	        <table width="100%">
