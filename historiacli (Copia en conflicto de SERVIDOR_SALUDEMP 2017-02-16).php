<?PHP
  ...
  if($opcion == "guardarcambio")
  {
    ...
    $teridempresa = $clase->BDLockup($empresa,'terceros','nit','terid');
    $contratoid   = $clase->SeleccionarUno("SELECT MAX(contratoid) FROM contratos WHERE terid = ".$teridempresa);
    ...
    $docuid = $clase->BDLockup($id,'historiacli','historiaid','docuid');
    $clase->EjecutarSQL("UPDATE documentos SET tipoexamen='".$tipoexamen."' , nitempresa='".$empresa."' , contratoid = ".$contratoid." WHERE docuid=".$docuid);
...
expuesto11,expuesto12,expuesto13,expuesto14,expuesto15,expuesto16,creador,momento) values(".$id.",'".$fecdesde."','".$fechasta."','".$empresa."','".$ocupacion."','".$tiempo."','".$riesgos."','".$expuesto1."','".$expuesto2."','".$expuesto3."','".$expuesto4."','".$expuesto5."','".$expuesto6."','".$expuesto7."','".$expuesto8."','".$expuesto9."','".$expuesto10."','".$expuesto11."','".$expuesto12."','".$expuesto13."','".$expuesto14."','".$expuesto15."','".$expuesto16."','".$_SESSION['USERNAME']."',CURRENT_TIMESTAMP)";
  ...
  if($opcion == "")
  {
	...
	if($vsql == "")
	{
    	$vsql = "SELECT HC.* , T1.nombre NombrePaciente , T2.nombre NombreMedico , 
    	         (SELECT T.nombre FROM terceros T INNER JOIN contratos C ON (C.terid = T.terid) INNER JOIN documentos D ON (D.contratoid = C.contratoid) WHERE D.docuid = HC.docuid ) NombreEntidad
    	         FROM historiacli HC 
		         INNER JOIN terceros T1 ON (HC.teridpaciente = T1.terid) 
    ...
    while($row = mysql_fetch_array($result)) 
	{
		 ...
		 $cont.=' <td width="10"> </td>
				  ...
				  <td width="60"> <font color="blue">'.substr($row['NombreEntidad'],0,12).' </td>
