<?PHP
  ...
  if(($opcion == "guardareditado")||($opcion == "guardarnuevo"))
  {
	...
	{
        $vsql = "INSERT INTO niveledu(codigo,descripcion,creador,momento) values('".$codigo."', 
	            '".$descripcion."','".$_SESSION['USERNAME']."',CURRENT_TIMESTAMP)";
    
		$cant = $clase->EjecutarSQL($vsql);
	
		if($cant == 1)
    		$clase->Aviso(1,"Nivel Educativo creado Exitosamente");  	
		else
	...
	{
        $vsql = "UPDATE niveledu SET codigo = '".$codigo."' , descripcion = '".$descripcion."' 
	             WHERE nivelid=".$nivelid;
    
	    $clase->EjecutarSQL($vsql);
	
   		$clase->Aviso(1,"Nivel Educativo modificado Exitosamente");  			  
    }	
  ...
  if($opcion == "nuevo")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/niveleducacion.png" width="32" height="32" border="0"> </td>
				 <td width="520"> Nuevo Nivel Educativo <td>
				 <td>  <a href="niveleducativo.php"> Listado de Niveles </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
  ...
  if($opcion == "detalles")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/niveleducacion.png" width="32" height="32" border="0"> </td>
				 <td width="510"> Niveles de Educacion <td>
				 <td>  <a href="niveleducativo.php"> Listado de Niveles </a> </td>
  	...
  	$vsql = "SELECT * FROM niveledu WHERE nivelid=".$id;
	...
	if($row = mysql_fetch_array($result))
	{
		$cont.='<br><br><br><center>
	        ...
	        <input type="hidden" name="nivelid" value="'.$id.'">
			<table width="400">
	         <tr> 
			  <td> <label class="Texto15"> Codigo : </label> </td>
  ...
  {
    $id = $_GET['id'];
    $vsql = "DELETE FROM niveledu WHERE nivelid=".$id;
	$clase->EjecutarSQL($vsql);
	$clase->Aviso(3,"Nivel Educativo Eliminado Exitosamente");  		
	header("Location: niveleducativo.php");
  }
  ...
  if($opcion == "encontrar")
  {
    ...
    $vsql = "SELECT * FROM niveledu WHERE codigo like '%".$criterio."%' OR descripcion like '%".$criterio."%' ORDER BY descripcion ASC Limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    $_SESSION['SQL_NIVELESEDU'] = $vsql;
	header("Location: niveleducativo.php");
  }
  ...
  {
    $criterio = $_POST['criterio'];
  	$vsql = "SELECT * FROM niveledu ORDER BY codigo ASC limit 0,30";
  ...
  if($opcion == "")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/niveleducacion.png" width="32" height="32" border="0"> </td>
				 <td width="400"> Niveles Educativos <td>
    ...
    $vsql = $_SESSION['SQL_NIVELESEDU'];
	if($vsql == "")
    	$vsql = "SELECT * FROM niveledu ORDER BY codigo ASC limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    ...
    while($row = mysql_fetch_array($result)) 
	{
		 ...
		 $cont.=' <td> </td>
				  ...
				  <td> </td>
				  <td> <a href="?opcion=detalles&amp;id='.$row['nivelid'].'"> <img src="images/seleccion.png" border="0"> </td>				  
				  <td> </td>
				 </tr>';
	}
	$cont.='</table>
