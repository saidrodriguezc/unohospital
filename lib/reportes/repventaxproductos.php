<?PHP
  ...
  if($opcion == "enpantalla")
  {
    ...
    $sqlventatotal = "SELECT SUM(DD.cantidad) FROM documentos  D INNER JOIN dedocumentos DD ON (D.docuid = DD.docuid)
			 INNER JOIN item I ON (DD.itemid = I.itemid) INNER JOIN productos P ON (P.itemid = I.itemid)			
			 WHERE tipodoc = 'PSE' AND (fechadoc >= '".$desde." 00:00:00' AND fechadoc <= '".$hasta." 23:29:00') 
			 AND fecasentado <> '0000-00-00 00:00:00' AND fecanulado = '0000-00-00 00:00:00' ";
    $ventatotal = $clase->SeleccionarUno($sqlventatotal);

	$vsql = "SELECT I.codigo Codproducto , I.descripcion Nomproducto, SUM(DD.cantidad) cant , SUM(DD.valparcial) total 
			 FROM documentos  D INNER JOIN dedocumentos DD ON (D.docuid = DD.docuid)
			 INNER JOIN item I ON (DD.itemid = I.itemid) INNER JOIN productos P ON (P.itemid = I.itemid)			
			 WHERE tipodoc = 'PSE' AND (fechadoc >= '".$desde." 00:00:00' AND fechadoc <= '".$hasta." 23:29:00') 
			 AND fecasentado <> '0000-00-00 00:00:00' AND fecanulado = '0000-00-00 00:00:00' ";

	$vsql.= "GROUP BY 1,2 ORDER BY 4 DESC ";
	
   	$cont = $clase->HeaderReportes();
   	$cont.= EncabezadoReporte2("Top de Servicios Mas Prestados - Mejor Rotacion");		 
   	$cont.='<center>
	        <b>Informe de Servicios Prestados</b> <br>
			...
			<table width="850">
    	      <tr class="CabezoteTabla"> 
                 <td width="15">  </td>
                 <td width="20" align="right"><b> Top </b></td>				 
                 <td width="15"></td>				 
    		     <td width="100"><b>CODIGO</B> </td>
    		     <td width="180"><b>PRODUCTO</B> </td>
				 <td width="50" align="right"> <b>CANT</B></td>
				 <td width="80" align="right"> <b>% REP</B></td>				 
                 <td width="15"> </td>					
			  </tr>
			</table>
	...
	while($row = mysql_fetch_array($result))  	
	{
        ...
        $representacion = (($row['cant']*100)/$ventatotal);           
		
		$cont.='<td width="15"> </td>
		         <td width="20" align="right"><b>'.number_format($i).'</b></td>
				 <td width="15"></td>
  	             <td width="70">'.$row['Codproducto'].'</td>
		         <td width="180">'.$row['Nomproducto'].'</td>
		         <td width="50" align="right">'.$row['cant'].'</td>
				 <td width="80" align="right"><b>'.number_format($representacion, 2,',','.').'</b></td>
				 <td width="15"> </td>
			    </tr>';
	...
	$cont.='</table>  
	        <table width="850"> 
			 <tr class="CabezoteTabla"> 
		      <td width="15"> </td>
	          <td width="360"> <b> Total General </b></td>
	          <td width="50" align="right"> <b>'.number_format($cantgral).'</b> </td>
		      <td width="95" align="right"> <b>'.number_format(100).'</b> </td>
		      <td width="17"> </td>
			 </tr>';    
  ...
  {
	 $cont = $clase->HeaderReportes();
     $cont.= EncabezadoReporte("Top de Servicios Prestados");		 
	 ...
	 $cont.='<script src="popcalendar.js" type="text/javascript"></script>
			 ...
			 <table width="600">
	           <tr class="CabezoteTabla"> 
				 ...
				 <td width="553"> Top de Servicios Prestados - Mejor Rotacion<td>
			  </tr>
			 </table>
			 <table width="600">
