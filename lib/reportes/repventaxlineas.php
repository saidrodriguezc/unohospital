<?PHP
  ...
  if($opcion == "enpantalla")
  {
	$desde   = $_GET['desde'];
	$hasta   = $_GET['hasta'];
	$lineaid = $_GET['lineaprodid'];

	$vsql = "SELECT LP.descripcion Linea , I.codigo Codproducto , I.descripcion Nomproducto, SUM(DD.cantidad) cant , SUM(DD.valparcial) total 
			 FROM documentos  D INNER JOIN dedocumentos DD ON (D.docuid = DD.docuid)
			 INNER JOIN item I ON (DD.itemid = I.itemid) INNER JOIN productos P ON (P.itemid = I.itemid)
			 INNER JOIN lineasprod LP ON (LP.lineaprodid = P.lineaprodid)
			 WHERE (tipodoc = 'FVE' OR tipodoc = 'RSA') AND (fechadoc >= '".$desde." 00:00:00' AND fechadoc <= '".$hasta." 23:29:00') 
			 AND fecasentado <> '0000-00-00 00:00:00' AND fecanulado = '0000-00-00 00:00:00' ";

	if($grupoid != "")		 
  	   $vsql.= "AND P.lineaprodid='".$lineaid."' ";
  	   
	$vsql.= "GROUP BY 1,2
	         ORDER BY LP.codigo ASC , I.descripcion ASC ";
 
   	$cont = $clase->HeaderReportes();
   	$cont.= EncabezadoReporte2("Informe de Ventas por Lineas de Productos");		 
   	$cont.='<center>
	        <b>Informe de Ventas por Lineas de Productos</b> <br>
	...
	while($row = mysql_fetch_array($result))  	
	{
		...
		if($i == 0)
		{
			$grupoactual = $row['Linea'];
			$nuevogrupo  = $row['Linea'];
		}
		else
		{
			if($row['Linea'] != $grupoactual)
			{
  ...
  {
	 $cont = $clase->HeaderReportes();
     $cont.= EncabezadoReporte("Informe de Ventas por Lineas de Productos");		 
	 ...
	 $cont.='<form action="?opcion=ver" method="POST" name="x">
			 <table width="600">
	           <tr class="CabezoteTabla"> 
				 ...
				 <td width="553"> Informe de Ventas por Lineas de Productos <td>
			  </tr>
			 </table>
			 <table width="600">
	           ...
	           <tr class="BarraDocumentos"> 
			     <td width="50">  </td>
			     <td width="80"> Linea :  </td>
			     <td width="160">'.$clase->CrearCombo("lineaprodid","lineasprod","descripcion","lineaprodid","","S").'</td>			     
				 <td width="50"> <td>
			  </tr>
			  </table>
			  <table width="600">
