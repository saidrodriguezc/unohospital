<?PHP
  ...
  if(($opcion == "guardareditado")||($opcion == "guardarnuevo"))
  {
	...
	{
        $vsql = "INSERT INTO bancos(codigo,descripcion,creador,momento) values('".$codigo."', 
	            '".$descripcion."','".$_SESSION['USERNAME']."',CURRENT_TIMESTAMP)";
    
		$cant = $clase->EjecutarSQL($vsql);
	
		if($cant == 1)
    		$clase->Aviso(1,"Banco creada Exitosamente");  	
		else
	...
	{
        $vsql = "UPDATE bancos SET codigo = '".$codigo."' , descripcion = '".$descripcion."' 
	             WHERE bancoid=".$bancoid;
				 
	    $clase->EjecutarSQL($vsql);
	
   		$clase->Aviso(1,"Banco modificado Exitosamente");  			  
    }	
  ...
  if($opcion == "nuevo")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
				 ...
				 <td width="520"> Nuevo Banco <td>
				 <td>  <a href="bancos.php"> Listado de Bancos </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
  ...
  if($opcion == "detalles")
  {
	 ...
	 $cont.='<table width="100%">
  	...
  	$vsql = "SELECT * FROM bancos WHERE bancoid=".$id;
	...
	if($row = mysql_fetch_array($result))
	{
		$cont.='<br><br><br><center>
	        ...
	        <input type="hidden" name="bancoid" value="'.$id.'">
			<table width="400">
	         <tr> 
			  <td> <label class="Texto15"> Codigo : </label> </td>
  ...
  {
    $id = $_GET['id'];
    $vsql = "DELETE FROM bancos WHERE especialidadid=".$id;
	$clase->EjecutarSQL($vsql);
	$clase->Aviso(3,"Banco Eliminado Exitosamente");  		
	header("Location: bancos.php");
  }
  ...
  if($opcion == "encontrar")
  {
    ...
    $vsql = "SELECT * FROM bancos WHERE codigo like '%".$criterio."%' OR descripcion like '%".$criterio."%' ORDER BY descripcion ASC Limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    $_SESSION['SQL_BANCOS'] = $vsql;
	header("Location: bancos.php");
  }
  ...
  {
    $criterio = $_POST['criterio'];
  	$vsql = "SELECT * FROM bancos ORDER BY descripcion ASC limit 0,30";
	$_SESSION['SQL_BANCOS'] = "";
	header("Location: bancos.php");
  }
  ...
  if($opcion == "")
  {
    ...
    $vsql = $_SESSION['SQL_BANCOS'];
	if($vsql == "")
    	$vsql = "SELECT * FROM bancos ORDER BY descripcion ASC limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    ...
    while($row = mysql_fetch_array($result)) 
	{
		 ...
		 $cont.=' <td width="10"> </td>
				  ...
				  <td width="100"> </td>
				  <td width="20"> <a href="?opcion=detalles&amp;id='.$row['bancoid'].'"> <img src="images/seleccion.png" border="0"> </td>				  
				 </tr>';
	}
	$cont.='</table>
	        <table width="100%">
