<?PHP
  ...
  /////////////////////////////////////////////////////////////////////////    
  if($opcion == "guardarhuella")
  {
   $terid = $_GET['terid'];
   $numero = mt_rand(1,9);   
   $cedula = $clase->BDLockup($terid,"terceros","terid","nit");   
   copy('images/basicas/huellas/'.$numero.'.jpg','huellas/'.$cedula.'.jpg');
   $vsql="UPDATE terceros SET rutahuella='".$cedula.".jpg' WHERE terid =".$terid;
   $clase->EjecutarSQL($vsql);
   $clase->Aviso(1,'Huella Dactilar Capturada Exitosamente'); 		    
   header('Location: pacientes.php?opcion=multimedia&terid='.$terid);
  } 
  
  /////////////////////////////////////////////////////////////////////////    
  if($opcion == "capturarhuella")
  {
   $terid = $_GET['terid'];
   echo'
    <center> 
    <h3> Instrucciones Toma Huella </h3>
	<p> Coloque correctamente el dedo indice sobre el dispositivo Digital Persona
	    y cuando decida capturar la imagen, haga click en el Boton <b>Capturar Huella</b></p>
    <br>
    <input type="button" class="button blue" value="Capturar Huella" OnClick="document.location=\'?opcion=guardarhuella&terid='.$terid.'\'">	
	<br><br><br>';
	exit();
  } 
  ...
  if($opcion == "guardarfoto2")
  {
     ...
     if (move_uploaded_file($HTTP_POST_FILES['userfile']['tmp_name'],'fotos/'.$nombrefinal))
	 { 
	     $clase->EjecutarSQL("UPDATE terceros SET rutafoto='".$nombrefinal."' WHERE terid=".$terid);	 
      	 $clase->Aviso(1,'Fotografia Asignada Exitosamente'); 		 
   	 }
	else
	{ 
         $clase->Aviso(2,'Error al Asignar la Firma'); 
  ...
  /////////////////////////////////////////  
  if($opcion == "cargarfoto2")
  {
    $id = $_GET['terid'];
    echo'<form action="pacientes.php?opcion=guardarfoto2" method="post" enctype="multipart/form-data"> 
   	 <b>Seleccione Archivo Foto</b> 
   	 <br> 
     <input name="userfile" type="file"> 
	 <br><br>
   	 <input type="hidden" name="MAX_FILE_SIZE" value="100000"> 
   	 <input type="hidden" name="terid" value="'.$id.'"> 	 
     <input type="submit" value="Cargar Foto desde PC">
	 </form>';
	exit();
  }	 
  ...
  if($opcion == "cargarfirma")
  {
   ...
   header('Location: pacientes.php?opcion=multimedia&terid='.$id);
  } 
  ...
  {
     $terid  = $_GET['terid'];
	 $nombre = $clase->BDlockup($terid,'terceros','terid','nombre');
	 ...
	 $cont.='<script src="popcalendar.js" type="text/javascript"></script>
	          <table width="100%">
	           <tr class="CabezoteTabla"> 
				 ...
				 <td width="160" align="left"> Multimedia del Paciente <td>
				 <td width="120" align="center"> <b>Paso 2</b> <td>				 
 				 <td width="300" align="right"> Paciente : <b>'.$nombre.'</b> <td>
				 <td width="24" align="right"> <input type="button" class="button" value="<<" OnClick="document.location=\'pacientes.php\'"></td>
			   </tr> </table> </form>';

	$cont.= FuncionesEspeciales(2);		 				 	
  	$vsql = "SELECT * FROM terceros WHERE terid=".$terid;
	...
	if($row = mysql_fetch_array($result))
	{
	 ...
	 if($row['rutafoto'] == "")
	   $imgfoto = '<img src="images/nofoto.png"  width="150" height="147" border="0" class="img">';
	 else
       $imgfoto = '<a href="fotos/'.$row['rutafoto'].'" rel="facebox">
	               <img src="fotos/'.$row['rutafoto'].'"  width="150" height="147" border="0" class="img"> </a>';
	 
	 if($row['rutafirma'] == "")
	   $imgfirma = '<img src="images/nofirma.png"  width="150" height="147" border="0" class="img">';
	 else
       $imgfirma = '<a href="firmas/'.$row['rutafirma'].'" rel="facebox"> 
	                <img src="firmas/'.$row['rutafirma'].'"  width="150" height="147" border="0" class="img"> </a>';

	 if($row['rutahuella'] == "")
	   $imghuella = '<img src="images/nohuella.png" width="120" height="127" border="0">';
	 else
       $imghuella = '<img src="huellas/'.$row['rutahuella'].'"  width="100" height="97" border="0" class="img"> </a>';
	 
	 $cont.='<br><center>
            <form action="?opcion=guardaracademica" method="POST" name="x">
			<input type="hidden" name="terid" value="'.$terid.'">
	        <center>			
            <h3>   Multimedia del Paciente  </h3>
            <br>			
			<table width="700">
	         <tr> 
              <td align="center" widht="33%"> 
			    <a href="#" onClick="window.open(\'tomarfoto/index.php?id=\',\'Foto\',\'width=720,height=430\');">
			    <img src="images/fotoestudiante.png" border="0"></a> </td>
              <td align="center" widht="33%"> 
			    <a href="#" onClick="window.open(\'tomarfirma/main_eng.swf\',\'Firma\',\'width=900,height=600\');">
				<img src="images/firmaestudiante.png" border="0"></a></td>
              <td align="center" widht="34%"> 
			    <a href="pacientes.php?opcion=capturarhuella&terid='.$terid.'" rel="facebox">
				<img src="images/huellaestudiante.png" border="0"> </a></td>
			 </tr>
			 <tr height="60"> 
              <td align="center"> <input type="button" value="Foto Tomada" class="button" Onclick="document.location=\'?opcion=cargarfoto&terid='.$terid.'\'"> 
			                  <a href="?opcion=cargarfoto2&terid='.$terid.'" rel="facebox"><input type="button" value="Desde PC" class="button blue"></a> </td>
              <td align="center"> <input type="button" value="Capturar Firma" class="button" Onclick="document.location=\'?opcion=cargarfirma&terid='.$terid.'\'">
			                  <a href="?opcion=cargarfirma2&terid='.$terid.'" rel="facebox"><input type="button" value="Desde PC" class="button blue"></a>  </td>
              <td align="center"> 
			     <a href="?opcion=capturarhuella&terid='.$terid.'" rel="facebox">
			      <input type="button" value="Capturar Huella" class="button"></a> </td>
			 </tr>
			 <tr> 
              <td align="center">'.$imgfoto.' </td>
              <td align="center">'.$imgfirma.' </td>
              <td align="center">'.$imghuella.'</td>
			 </tr>
  ...
  function FuncionesEspeciales($item)
  {
	 ...
	 if($id == "")
  	 ...
  	 $cont.='<table width="100%">	
	          <tr class="BarraDocumentos">';
     if($item == 1)			  
	    $cont.='<td width="20%" class="BarraDocumentosSel" align="center"> <b> Datos Basicos </td>';
     else
	    $cont.='<td width="20%" align="center"> <a href="?opcion=detalles&id='.$id.'">Datos Basicos </a></td>';
     if($item == 2)			  
	    $cont.='<td width="20%" class="BarraDocumentosSel" align="center"><b> Multimedia </td>';
     else
	    $cont.='<td width="20%" align="center"> <a href="?opcion=multimedia&terid='.$id.'">Multimedia </a> </td>';
