<?PHP
  ...
  if(($opcion == "guardareditado")||($opcion == "guardarnuevo"))
  {
	...
	{
        $vsql = "INSERT INTO especialidades(codigo,descripcion,creador,momento) values('".$codigo."', 
	            '".$descripcion."','".$_SESSION['USERNAME']."',CURRENT_TIMESTAMP)";
    
		$cant = $clase->EjecutarSQL($vsql);
	
		if($cant == 1)
    		$clase->Aviso(1,"Especialidad creada Exitosamente");  	
		else
	...
	{
        $vsql = "UPDATE especialidades SET codigo = '".$codigo."' , descripcion = '".$descripcion."' 
	             WHERE especialidadid=".$especialidadid;
				 
	    $clase->EjecutarSQL($vsql);
	
   		$clase->Aviso(1,"Especialidad modificada Exitosamente");  			  
    }	
  ...
  if($opcion == "nuevo")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/especialidades.png" width="32" height="32" border="0"> </td>
				 <td width="520"> Nueva Especialidad <td>
				 <td>  <a href="especialidades.php"> Listado de Especialidades </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
  ...
  if($opcion == "detalles")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/especialidades.png" width="32" height="32" border="0"> </td>
				 <td width="510"> Especialidades de Profesionales <td>
				 <td>  <a href="especialidades.php"> Listado de Especialidades </a> </td>
    		     <td width="8"> </td>
			   </tr>	 			   
			 </table> ';	
  	...
  	$vsql = "SELECT * FROM especialidades WHERE especialidadid=".$id;
	...
	if($row = mysql_fetch_array($result))
	{
		$cont.='<br><br><br><center>
	        ...
	        <input type="hidden" name="especialidadid" value="'.$id.'">
			<table width="400">
	         <tr> 
			  <td> <label class="Texto15"> Codigo : </label> </td>
  ...
  {
    $id = $_GET['id'];
    $vsql = "DELETE FROM especialidades WHERE especialidadid=".$id;
	$clase->EjecutarSQL($vsql);
	$clase->Aviso(3,"Especialidad Eliminada Exitosamente");  		
	header("Location: especialidades.php");
  }
  ...
  if($opcion == "encontrar")
  {
    ...
    $vsql = "SELECT * FROM especialidades WHERE codigo like '%".$criterio."%' OR descripcion like '%".$criterio."%' ORDER BY codigo ASC Limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    $_SESSION['SQL_ESPECIALIDADES'] = $vsql;
	header("Location: especialidades.php");
  }
  ...
  {
    $criterio = $_POST['criterio'];
  	$vsql = "SELECT * FROM especialidades ORDER BY codigo ASC limit 0,30";
	$_SESSION['SQL_ESPECIALIDADES'] = "";
	header("Location: especialidades.php");
  }
  ...
  if($opcion == "")
  {
	 ...
	 $cont.='<table width="100%">
	           <tr class="CabezoteTabla"> 
			     <td width="10"> </td>
			     <td width="37"> <img src="images/iconos/especialidades.png" width="32" height="32" border="0"> </td>
				 <td width="400"> Especialidades de Profesionales<td>
	 ...
	 if($_SESSION['SQL_ESPECIALIDADES'] != "")
    ...
    $vsql = $_SESSION['SQL_ESPECIALIDADES'];
	if($vsql == "")
    	$vsql = "SELECT * FROM especialidades ORDER BY codigo ASC limit 0,".$_SESSION["NUMREGISTROSXCONSULTA"];
    ...
    while($row = mysql_fetch_array($result)) 
	{
		 ...
		 $cont.=' <td width="10"> </td>
				  ...
				  <td width="100"> </td>
				  <td width="20"> <a href="?opcion=detalles&amp;id='.$row['especialidadid'].'"> <img src="images/seleccion.png" border="0"> </td>				  
				 </tr>';
	}
	$cont.='</table>
	        <table width="100%">
