<?PHP
  ...
  if($opcion == "enpantalla")
  {
	...
	$vsql = "SELECT EC.nombre Grupo , I.codigo Codproducto , I.descripcion Nomproducto, SUM(DD.cantidad) cant
			 FROM documentos  D INNER JOIN dedocumentos DD ON (D.docuid = DD.docuid)
			 INNER JOIN item I ON (DD.itemid = I.itemid) INNER JOIN productos P ON (P.itemid = I.itemid)
	...
	$vsql.= "GROUP BY 1,2
	         ORDER BY C.descripcion ASC , I.descripcion ASC ";
	         
   	$cont = $clase->HeaderReportes();
   	$cont.= EncabezadoReporte2("Resumen del Prestador");		 
   	$cont.='<center>
	        <b>Detalle de Servicios por Contrato</b> <br>
  ...
  if($opcion == "ver")
  {
	...
	////////////////////////////////////////
    if($formato == 'html')
      header("Location: ?opcion=enpantalla&desde=".$desde."&hasta=".$hasta."&prestador=".$terid." ");	
  ...
  {
	 $cont = $clase->HeaderReportes();
     $cont.= EncabezadoReporte("Informe de Servicios por Prestador");		 
